...
def train_CIFAR10DVS(config):
    ...
    frame_transform = transforms.Compose([
        tonic.transforms.CropTime(max=50000),
        transforms.ToTimesurface(
            sensor_size=sensor_size, surface_dimensions=None, tau=config["tau"],
            decay="exp"
        )
    ])

    dataset = tonic.datasets.CIFAR10DVS(save_to='/home/hubo1024/PycharmProjects/snn_paper/data/CIFAR10DVS',
    ...
    trainset, valset, testset = random_split(dataset, [train_size, validation_size, test_size], generator=torch.Generator().manual_seed(1))
...
def test_best_model(best_result):
    ...
    frame_transform = transforms.Compose([
        tonic.transforms.CropTime(max=50000),
        transforms.ToTimesurface(
            sensor_size=sensor_size, surface_dimensions=None, tau=best_result.config["tau"],
            decay="exp"
        )
    ])
    dataset = tonic.datasets.CIFAR10DVS(save_to='/home/hubo1024/PycharmProjects/snn_paper/data/CIFAR10DVS',
...
def main(num_samples=10, max_num_epochs=20, gpus_per_trial=2):
    ...
    bayesopt = BayesOptSearch(space, metric="accuracy", mode="max")
    scheduler = AsyncHyperBandScheduler(max_t=max_num_epochs)
    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(train_CIFAR10DVS),
    ...
    #tuner = tune.Tuner.restore("/home/hubo1024/ray_results/train_CIFAR10DVS_2022-11-14_13-59-37")
    results = tuner.fit()
...
if __name__ == "__main__":
    ...
    main(num_samples=100, max_num_epochs=10, gpus_per_trial=1)
